...
export const fetchLinks = async (useCache = true) => {
  if (useCache) {
    const cached = getCachedData('links');
    if (cached) {
      return cached;
    }
  }
  ...
  const url = API_KEY ? `${API_URL}?key=${encodeURIComponent(API_KEY)}` : API_URL;
  const response = await fetch(url, {
    method: 'GET',
  });
  const data = await handleResponse(response);
  if (data) {
    setCachedData('links', data);
  }
  return data;
};
...
export const updateLink = async (payload) => {
  return createLink({ method: 'update', ...payload });
};

export const deleteLink = async (id) => {
  return createLink({ method: 'delete', id });
};
